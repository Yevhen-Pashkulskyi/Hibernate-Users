...
public class UserRepository implements AppRepository<User> {
    ...
    public String create(User contact) {
        ...
        try (Session session = HibernateUtil.getSessionFactory().openSession()) {
            ...
            String hql = "INSERT INTO User (firstName, email) " +
            ...
            return UsersConstant.DATA_INSERT_MSG;
        } catch (Exception e) {
            if (transaction != null) {
    ...
    public String update(User contact) {
        ...
        if (readById(contact.getId()).isEmpty()) {
            return UsersConstant.DATA_ABSENT_MSG;
        } else {
            Transaction transaction = null;
            try (Session session = HibernateUtil.getSessionFactory().openSession()) {
                // Транзакция стартует
                ...
                // :[parameter name] - іменований параметр (named parameter),
                // двокрапка перед іменем.
                String hql = "UPDATE User SET firstName = :firstName, email = :email WHERE id = :id";
                // Створення HQL-запиту
                MutationQuery query = session.createMutationQuery(hql);
                // Формування конкретних значень для певного іменованого параметра
                query.setParameter("firstName", contact.getFirstName());
                ...
                // Повернення повідомлення при безпомилковому
                // виконанні транзакції
                return UsersConstant.DATA_UPDATE_MSG;
            } catch (Exception e) {
                if (transaction != null) {
                    // Відкочення поточної транзакції ресурсу
                    transaction.rollback();
    ...
    public String delete(Long id) {
        ...
        if (readById(id).isEmpty()) {
            ...
            try (Session session = HibernateUtil.getSessionFactory().openSession()) {
                ...
                // Повернення повідомлення при безпомилковому
                // виконанні транзакції
                return UsersConstant.DATA_DELETE_MSG;
            } catch (Exception e) {
                if (transaction != null) {
                    // Відкочення поточної транзакції ресурсу
                    transaction.rollback();
